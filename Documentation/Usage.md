...
```
If we suppose all the phoneNumber values just contains number, and the phone number for each cantact is unique, we can set uniqueValue this way too:

```swift
var uniqueValue: Int {
	return Int(self.phoneNumber)!
}
```
